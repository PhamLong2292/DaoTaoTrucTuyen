    ...
    public class PhieuBaoCaoSuCoYKhoaProcessBll : PhieuBaoCaoSuCoYKhoaTemplate
    {
        ...
        public override PhieuBaoCaoSuCoYKhoaCls[] PageReading(ActionSqlParamCls ActionSqlParam, PhieuBaoCaoSuCoYKhoaFilterCls OPhieuBaoCaoSuCoYKhoaFilter, ref long recordTotal)
        {
            ...
            try
            {
                ...
                {
                    Query += " and (UPPER(MASOPHIEU) like UPPER('%" + OPhieuBaoCaoSuCoYKhoaFilter.Keyword + "%') OR UPPER(HOTEN) like UPPER(N'%" + OPhieuBaoCaoSuCoYKhoaFilter.Keyword + "%') OR UPPER(MABN) like UPPER('%" + OPhieuBaoCaoSuCoYKhoaFilter.Keyword + "%'))";
                    recordTotalQuery += " and (UPPER(MASOPHIEU) like UPPER('%" + OPhieuBaoCaoSuCoYKhoaFilter.Keyword + "%') OR UPPER(HOTEN) like UPPER(N'%" + OPhieuBaoCaoSuCoYKhoaFilter.Keyword + "%') OR UPPER(MABN) like UPPER('%" + OPhieuBaoCaoSuCoYKhoaFilter.Keyword + "%'))";
                }
                if (OPhieuBaoCaoSuCoYKhoaFilter.DoiTuongSuCo != null)
                {
                    ColDbParams.Add(new DbParam("DOITUONGSUCO", OPhieuBaoCaoSuCoYKhoaFilter.DoiTuongSuCo));
        ...
        public override PhieuBaoCaoSuCoYKhoaCls CreateModel(ActionSqlParamCls ActionSqlParam, string ID)
        {
            ...
            {
                DataSet dsResult =
                        DBService.GetDataSet(ActionSqlParam.Trans, "select * from PHIEUBAOCAOSUCOYKHOA where (ID =" + ActionSqlParam.SpecialChar + "ID or MASOPHIEU =" + ActionSqlParam.SpecialChar + "ID or MABN =" + ActionSqlParam.SpecialChar + "ID)", new DbParam[]{
                    new DbParam("ID",ID)
                    });
