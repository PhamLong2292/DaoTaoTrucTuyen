    ...
    public class DonViCongTacService : RemoteDataTemplate
    {
        ...
        public override AjaxOut Reading(RenderInfoCls ORenderInfo, int PageIndex, string Keyword)
        {
            ...
            try
            {
                ...
                var ajaxOut = OneMES3.DM.Call.Bussiness.Utility.CallBussinessUtility.CreateBussinessProcess().CreateDonViCongTacProcess().Count(Model.Common.CreateRenderInfo(ORenderInfo), ODonViCongTacFilter);
                long recordTotal = (long)ajaxOut;
                //long recordTotal = 0;
                var DonViCongTacs = OneMES3.DM.Call.Bussiness.Utility.CallBussinessUtility.CreateBussinessProcess().CreateDonViCongTacProcess().PageReading(Model.Common.CreateRenderInfo(ORenderInfo), ODonViCongTacFilter, ref recordTotal);
                ...
                for (int iIndex = 0; iIndex < DonViCongTacs.Length; iIndex++)
                {
                    ...
                    ORecord.items[iIndex].id = DonViCongTacs[iIndex].Id;
