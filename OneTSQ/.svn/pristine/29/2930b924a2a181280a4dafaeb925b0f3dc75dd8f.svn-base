...
namespace OneTSQ.ReportUtility
{
    ...
    public static class StreamHelper
    {
        ...
        public static byte[] DeflateDecompress(byte[] input)
        {
            using (MemoryStream output = new MemoryStream(input))
            {
                using (DeflateStream zip = new DeflateStream(output, CompressionMode.Decompress))
                {
                    List<byte> bytes = new List<byte>();
                    int b = zip.ReadByte();
                    while (b != -1)
        ...
        public static Stream ReadToStream(System.Drawing.Image image)
        {
        ...
        public static Stream ReadToStream(System.Drawing.Icon icon)
        {
            var stream = new MemoryStream();
            icon.Save(stream);
            stream.Seek(0, SeekOrigin.Begin);
        ...
        public static byte[] Read(Stream stream)
        {
            using (stream)
            {
                ...
                {
                    using (var output = new MemoryStream(32767))
                    {
                        stream.CopyTo(output, 51200);
                        return output.ToArray();
                    }
                }
            }
