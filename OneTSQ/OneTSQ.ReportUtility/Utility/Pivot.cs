...
namespace OneTSQ.ReportUtility.Utility
{
    public class Pivot
    {
        ...
        public DataTable PivotData(string DataField, AggregateFunction Aggregate, string[] RowFields, string[] ColumnFields)
        {
            ...
            foreach (var RowName in RowList)
            {
                ...
                foreach (string Field in RowFields)
                {
                    ...
                    if (RowName[Field].ToString().Contains("'"))
                    {
                        strFilter += string.Format(" and {0} = '{1}'", Field, RowName[Field].ToString().Replace("'", "''"));
                    }
                    else
                    {
                        //nếu giá trị RowName[Field] là rỗng thì phải hoặc thêm is null nữa, không thì sót, dẫn đến cột số lượng không lên
                        if (RowName[Field].ToString() == "" || RowName[Field] == null)
                        {
                            strFilter += string.Format(" and ({0} = '' or {0} is null)", Field);
                        }
                        else
                        {
                            strFilter += string.Format(" and {0} = '{1}'", Field, RowName[Field]);
                        }
                    }
                }
        ...
        private object GetData(string Filter, string DataField, AggregateFunction Aggregate)
        {
            try
            {
                ...
                switch (Aggregate)
                {
                    ...
                    case AggregateFunction.Aggregate:
                        return GetAggregate(objList);
                    default:
                        return null;
                }
            }
        ...
        private object GetAggregate(object[] objList)
        {
            return (objList.Count() == 0 || objList.Where(x => x != null).Count() == 0) ? null : (object)(objList.Where(x => x != null).Aggregate((x, y) => (x += y.ToString())));
        }
    }
