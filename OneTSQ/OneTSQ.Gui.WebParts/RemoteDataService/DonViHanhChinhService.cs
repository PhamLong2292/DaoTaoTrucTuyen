    ...
    public class DonViHanhChinhService : RemoteDataTemplate
    {
        ...
        public override AjaxOut Reading(RenderInfoCls ORenderInfo, int PageIndex, string Keyword)
        {
            ...
            try
            {
                ...
                var ajaxOut = OneMES3.DM.Call.Bussiness.Utility.CallBussinessUtility.CreateBussinessProcess().CreateDonViHanhChinhProcess().Count(Model.Common.CreateRenderInfo(ORenderInfo), ODonViHanhChinhFilter);
                long recordTotal = (long)ajaxOut;
                //long recordTotal = 0;
                var DonViHanhChinhs = OneMES3.DM.Call.Bussiness.Utility.CallBussinessUtility.CreateBussinessProcess().CreateDonViHanhChinhProcess().PageReading(Model.Common.CreateRenderInfo(ORenderInfo), ODonViHanhChinhFilter, ref recordTotal);
