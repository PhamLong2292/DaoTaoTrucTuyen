    ...
    public class IcdService : RemoteDataTemplate
    {
        ...
        public override AjaxOut Reading(RenderInfoCls ORenderInfo, int PageIndex, string Keyword)
        {
            ...
            try
            {
                ...
                var ajaxOut = OneMES3.DM.Call.Bussiness.Utility.CallBussinessUtility.CreateBussinessProcess().CreateIcdProcess().Count(Model.Common.CreateRenderInfo(ORenderInfo), OIcdFilter);
                int recordTotal = (int)ajaxOut;
                //long recordTotal = 0;
                var Icds = OneMES3.DM.Call.Bussiness.Utility.CallBussinessUtility.CreateBussinessProcess().CreateIcdProcess().PageReading(Model.Common.CreateRenderInfo(ORenderInfo), OIcdFilter, ref recordTotal);
