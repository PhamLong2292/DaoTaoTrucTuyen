    ...
    public class ChuyenMonService : RemoteDataTemplate
    {
        ...
        public override AjaxOut Reading(RenderInfoCls ORenderInfo, int PageIndex, string Keyword)
        {
            ...
            try
            {
                ...
                var ajaxOut = OneMES3.DM.Call.Bussiness.Utility.CallBussinessUtility.CreateBussinessProcess().CreateChuyenMonProcess().Count(Model.Common.CreateRenderInfo(ORenderInfo), OChuyenMonFilter);
                int recordTotal = (int)ajaxOut;
                //long recordTotal = 0;
                var chuyenMons = OneMES3.DM.Call.Bussiness.Utility.CallBussinessUtility.CreateBussinessProcess().CreateChuyenMonProcess().PageReading(Model.Common.CreateRenderInfo(ORenderInfo), OChuyenMonFilter, ref recordTotal);
