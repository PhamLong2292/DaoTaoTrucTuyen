...
namespace OneTSQ.WebParts
{
    public class DT_LopHocs : WebPartTemplate
    {
        ...
        public override AjaxOut Draw(SiteParam OSiteParam, RenderInfoCls ORenderInfo)
        {
            ...
            try
            {
                ...
                RetAjaxOut.HtmlContent =
                    ...
                    "       document.title = '" + WebLanguage.GetLanguage(OSiteParam, "OneTSQ: Danh sách lớp học") + "';\r\n" +
        ...
        private static string GetDataPermissionQuery(RenderInfoCls ORenderInfo)
        {
            SiteParam OSiteParam = WebEnvironments.CreateSiteParam(ORenderInfo);
            OwnerUserCls user = WebSessionUtility.GetCurrentLoginUser(OSiteParam);
            bool xemPermission = user.IsSystemAdmin == 1 || PermissionUtility.CheckPermission(ORenderInfo, new DT_LopHocPermission().PermissionFunctionCode, DT_KhoaHocCls.eLopHocPermission.Xem.ToString(), new DT_LopHocPermission().PermissionFunctionCode, DT_LopHocPermission.StaticPermissionFunctionId, user.OwnerUserId);
